	...
	tests := []struct {
		...
		},
		{
			...
			validateActions: func(t *testing.T, actions []Action) {
				...
				if deleteAction.Pod.Meta.Node != "node1" {
					t.Errorf("Expected pod node to be 'node1', got '%s'", deleteAction.Pod.Meta.Node)
				}
				...
				if createAction.Pod.Meta.Node != "node2" {
					t.Errorf("Expected pod node to be 'node2', got '%s'", createAction.Pod.Meta.Node)
				}
			},
		},
		{
